...
//        return {true, current_term};
//    }
//    auto entry = log_storage->get_nth(args.prev_log_index);
//    if(entry.term_id == args.prev_log_term || args.prev_log_index == 0){
...
//        std::cout<<"next index of "<<node_id<<"  is  :    "<<arg.prev_log_index<<" \n";
//        next_index[node_id] = arg.prev_log_index;
//        return;
//    }
//    // if accept
//    std::cout<<"handle_append_entries_reply "<<my_id<<" "<<reply.term<<" "<< current_term<<std::endl;
//    if (arg.alive) {
//        fixed_timer.receive();
//        return;
//    }
//    auto last_included_idx = get_last_include_idx();
//    auto agree_idx = arg.prev_log_index + arg.entries.size();
//    const auto cnt = node_configs.size();
//    next_index[node_id] = agree_idx + 1;
...
//        }
//        ++applied_map[idx];
//        if (idx <= commit_idx || applied_map[idx] > cnt / 2) {
    ...
    while (true) {
        {
            ...
            for (int idx = last_apply - last_included_idx;
                 idx <= this_applied - last_included_idx;
                 ++idx) {
                if (idx < 0) continue;
